...
int main (int argc, char *argv[]){
      ...
      pthread_t totalgralThread; //total gral
      pthread_attr_t attr;
     ...
     //  pthread_create(&totalgralThread,&attr,totalGeneral,NULL); //	Creo hilo para totalgral
...
void *sumoVentas(void *sucursalid) {
   ...
   pthread_cond_signal(&totalgralCond); 
...
void *totalGeneral(void *p) {
   static int veces = 0;

	while (veces < 5){
		pthread_mutex_lock(&totalgralMtx);
		pthread_cond_wait(&totalgralCond,&totalgralMtx);
		veces++;
	}
	printf("total gral \n");
   pthread_exit(NULL);
